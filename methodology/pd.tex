\subsubsection{Design of a PID-Based HAC}

Proportional integral derivative (PID) shown in \cref{eq:pid} can be applied for making a simple aviation human-AI collaboration (HAC) agent in piloting the quadcopter. 
...
Using Unity's ToAngleAxis method, this relative rotation is converted into an angle $\phi$ and an axis $\mathbf{n}$. The error vector $\mathbf{e}$ is then $\mathbf{e} = \phi \cdot \mathbf{n}$. The controller applies a PID loop to generate corrective torque shown in \cref{eq:pdt}.
...
The Inner Loop controls horizontal movement by calculating rolling and pitching moments ($\tau_x$ and $\tau_y$). It uses the quadcopter's current attitude and reference signals to generate these outputs \parencite{paredes2021quadcopter}. Meanwhile, the Outer Loop determines the necessary thrust, yawing moments, and reference signals.
