...
Additionally, a 2D minimap (HUD) was introduced to support spatial awareness by indicating the player's current position and the target location. To better constrain the navigation route while preserving environmental fidelity, the previous box collider was replaced with a mesh collider aligned with the terrain and structures.
...
\begin{figure}[H]
    \caption{The Tallinn Old Town model (top) and terrain (bottom) imported into Unity}
    \label{fig:tallinn}
    \centering
...
\begin{figure}[H]
    \caption{Navigation route (top), player view (bottom-left), and coin collection with mesh collider (bottom-right)}\label{fig:realistic}
    \centering
